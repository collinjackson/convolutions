...
//! constructor
CVideoDirectX9::CVideoDirectX9(const core::dimension2d<s32>& screenSize, HWND window, 
...
: CVideoNull(io, screenSize), D3DLibrary(0), CurrentRenderMode(ERM_NONE), pID3DDevice(0),
 ...
 DeviceLost(false), Fullscreen(fullscreen)
{

	#ifdef _DEBUG
	setDebugName("CVideoDirectX9");
	#endif

	os::Printer::log("DirectX9 Renderer", ELL_INFORMATION);
...
bool CVideoDirectX9::initDriver(const core::dimension2d<s32>& screenSize, HWND hwnd,
								u32 bits, bool fullScreen, bool pureSoftware)
{
	...
	present.SwapEffect					= D3DSWAPEFFECT_FLIP; // 3DSWAPEFFECT_COPY
	...
	if (!pID3DDevice)
	{
		os::Printer::log("Was not able to create DirectX9 device.", ELL_ERROR);
		return false;
	}
...
bool CVideoDirectX9::beginScene(bool backBuffer, bool zBuffer, SColor color)
{
	...
	if (FAILED(hr))
		os::Printer::log("DirectX9 clear failed.", ELL_WARNING);

	hr = pID3DDevice->BeginScene();
	if (FAILED(hr))
	{
		os::Printer::log("DirectX9 begin scene failed.", ELL_WARNING);
		return false;
	}
...
bool CVideoDirectX9::endScene()
{
	...
	if (FAILED(hr))
	{
		os::Printer::log("DirectX9 end scene failed.", ELL_WARNING);
		return false;
	}
	...
	{
		DeviceLost = true;
		os::Printer::log("DirectX9 device lost.", ELL_WARNING);
	}
	else
	if (FAILED(hr) && hr != D3DERR_INVALIDCALL)
	{
		os::Printer::log("DirectX9 present failed.", ELL_WARNING);
		return false;
	}
...
bool CVideoDirectX9::queryFeature(E_VIDEO_DRIVER_FEATURE feature)
{
	switch (feature)
	{
...
//! sets transformation
void CVideoDirectX9::setTransform(E_TRANSFORMATION_STATE state, const core::matrix4& mat)
{
...
void CVideoDirectX9::setTexture(s32 stage, video::ITexture* texture)
{
	if (CurrentTexture[stage] == texture)
		return;

	if (texture && texture->getDriverType() != EDT_DIRECTX9)
	{
	...
	{
		pID3DDevice->SetTexture(stage, ((CDirectX9Texture*)texture)->getDX9Texture());
		texture->grab();
	}
}
...
//! sets a material
void CVideoDirectX9::setMaterial(const SMaterial& material)
{
	Material = material;
...
video::ITexture* CVideoDirectX9::createDeviceDependentTexture(IImage* surface)
{
	...
	return new CDirectX9Texture(surface, pID3DDevice,
		TextureCreationFlags);
}


//! Enables or disables a texture creation flag.
void CVideoDirectX9::setTextureCreationFlag(E_TEXTURE_CREATION_FLAG flag, bool enabled)
{
...
void CVideoDirectX9::setRenderTarget(video::ITexture* texture)
{
...
//! sets a viewport
void CVideoDirectX9::setViewPort(const core::rect<s32>& area)
{
...
const core::rect<s32>& CVideoDirectX9::getViewPort() const
{
	return ViewPort;
}
...
void CVideoDirectX9::drawIndexedTriangleList(const S3DVertex* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
...
//! draws an indexed triangle list
void CVideoDirectX9::drawIndexedTriangleList(const S3DVertex2TCoords* vertices, s32 vertexCount, const u16* indexList, s32 triangleCount)
{
	if (!checkPrimitiveCount(vertexCount))
		return;
...
//! Draws an indexed triangle list.
void CVideoDirectX9::drawIndexedTriangleFan(const S3DVertex2TCoords* vertices,
...
void CVideoDirectX9::drawIndexedTriangleFan(const S3DVertex* vertices,
	s32 vertexCount, const u16* indexList, s32 triangleCount)
{
	if (!checkPrimitiveCount(vertexCount))
		return;
...
//! draws an 2d image
void CVideoDirectX9::draw2DImage(video::ITexture* texture, const core::position2d<s32>& destPos)
{
	if (!texture)
		return;
...
void CVideoDirectX9::draw2DImage(video::ITexture* texture, const core::position2d<s32>& pos, 
...
void CVideoDirectX9::draw2DRectangle(SColor color, const core::rect<s32>& position,
...
void CVideoDirectX9::setVertexShader(E_VERTEX_TYPE newType)
{
...
void CVideoDirectX9::setRenderStates3DMode()
{
	if (!pID3DDevice)
		return;
	...
	// disable mipmap levels if there are none in the texture
	if (ResetRenderStates && (Material.BilinearFilter || Material.TrilinearFilter))
	{
		for (s32 i=0 ;i<MATERIAL_MAX_TEXTURES; ++i)
		{
			if (Material.Textures[i] && CurrentTexture[i] &&
				!((CDirectX9Texture*)CurrentTexture[i])->hasMipMaps())
				pID3DDevice->SetSamplerState(i, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
		}
	}
...
//! sets the needed renderstates
void CVideoDirectX9::setRenderStatesStencilShadowMode(bool zfail)
{
...
//! sets the needed renderstates
void CVideoDirectX9::setRenderStatesStencilFillMode(bool alpha)
{
...
//! sets the needed renderstates
void CVideoDirectX9::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
{
	if (!pID3DDevice)
		return;
...
void CVideoDirectX9::deleteAllDynamicLights()
{
...
s32 CVideoDirectX9::getMaximalDynamicLightAmount()
{
...
void CVideoDirectX9::setAmbientLight(const SColorf& color)
{
	if (!pID3DDevice)
		return;
...
const wchar_t* CVideoDirectX9::getName()
{
	return L"Direct3D9";
}
...
void CVideoDirectX9::drawStencilShadowVolume(const core::vector3df* triangles, s32 count, bool zfail)
{
...
void CVideoDirectX9::drawStencilShadow(bool clearStencilBuffer, video::SColor leftUpEdge,
...
//! call.
s32 CVideoDirectX9::getMaximalPrimitiveCount()
{
...
//! Sets the fog mode.
void CVideoDirectX9::setFog(SColor color, bool linearFog, f32 start, 
...
//! Draws a 3d line.
void CVideoDirectX9::draw3DLine(const core::vector3df& start,
...
void CVideoDirectX9::OnResize(const core::dimension2d<s32>& size)
{
	if (!pID3DDevice)
		return;
...
{
    #ifdef _IRR_COMPILE_WITH_DIRECTX_9_
	CVideoDirectX9* dx9 =  new CVideoDirectX9(screenSize, window, fullscreen, stencilbuffer, io, pureSoftware);
	if (!dx9->initDriver(screenSize, window, bits, fullscreen, pureSoftware))
	{
		dx9->drop();
		dx9 = 0;
	}
