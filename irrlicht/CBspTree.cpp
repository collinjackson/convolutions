...
#ifdef _DEBUG
	c8 tmp[255];
	os::Printer::log("/* Bsp Tree Creation stats block--------------", ELL_INFORMATION);
...
bool CBspTree::findSplitter(BspPoly* poly, u32 polyCount, core::plane3d<f32>& outSplitter)
{
	...
	for (u32 i=0; i<polyCount; ++i)
	{
		...
		if (!poly[i].wasSplitter)
		{
			...
			s32 score = abs(frontCount - backCount) + (splitCount*3) + planarCount;
...
void CBspTree::renderBoxCulling(BspNode& node, const SViewFrustrum* camArea, const core::vector3df& camPos)
{
	...
	if (!node.bbox.intersectsWithBox(camArea->getBoundingBox()))
     return;
						 ...
						 BspPoly& frontSplit, BspPoly& backSplit)
{
	...
	for (u32 currentIdx2=0; currentIdx2<poly.vertices.size(); ++currentIdx2)
	{
			...
			plane.getIntersectionWithLimitedLine(pointA, pointB, intersection))
		{
			...
			f32 percent = (f32)(l2.getLength() / l.getLength());
			...
			if (!(percent < 0.0f || percent > 1.0f))
			{
				...
				video::SColor tmpcolor(0, red, green, blue);
	...
	u32 i; // new ISO for scoping not liked by some compilers

	for (i=0; i<countFront; ++i)
	...
	for (i=0; i<(u32)frontIndexCount/3; ++i)
	{
		if (i) {	v1=v2; v2++; }
	...
	for (i=0; i<(u32)frontIndexCount; ++i)
		if (frontSplit.indices[i] >= frontSplit.vertices.size())
		{
			os::Printer::log("[CBspTree] Error! Invalid index created during split.\n", ELL_ERROR);
			_asm int 3
		}
	#endif
	...
	for (i=0; i<(u32)backIndexCount/3; ++i)
	{
		if (i)
		{
			v1=v2;
	...
	#ifdef _DEBUG // falsche indizes suchen
	for (i=0; i<(u32)backIndexCount; ++i)
