...
namespace scene
{
	...
	*/
	...
	public:
		...
		//! constructor
		CSceneManager(video::IVideoDriver* driver, io::IFileSystem* fs, gui::ICursorControl* cursorControl);
		...
		//! Adds a camera scene node which is able to be controled with the mouse and keys
		//! like in most first person shooters (FPS): 
		virtual ICameraSceneNode* addCameraSceneNodeChase(ISceneNode* parent, 
			f32 rotateSpeed, f32 zoomSpeed, f32 translationSpeed, s32 id);

		
		virtual ICameraSceneNode* addCameraSceneNodeChaseOver(ISceneNode* parent = 0,
			f32 rotateSpeed = 1500.0f, f32 moveSpeed = 200.0f, s32 id=-1,
			SKeyMap* keyMapArray=0, s32 keyMapSize=0);
		...
		//! Adds a skybox fader scene node. It covers up the skybox with a solid color.
		virtual ISkyBoxFaderSceneNode* addSkyBoxFaderSceneNode(ISceneNode* parent = 0, s32 id=-1);
		...
		virtual ITerrainSceneNode* addTerrainSceneNode(ISceneNode* parent, s32 id,
			video::IImage* texture,	video::IImage* heightmap, video::ITexture* detailmap, 
	...
	private:
		...
		core::array<ISceneNode*> SkyBoxFaderList;
