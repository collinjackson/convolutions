...
EANIMATED_MESH_TYPE CXAnimationPlayer::getMeshType() const
{
	return EAMT_X;
}
...
void CXAnimationPlayer::createAnimationData()
{
	...
	if (IsAnimatedSkinnedMesh && AnimatedMesh)
	{
		...
		modifySkin();	
		updateBoundingBoxFromAnimation();
		DebugSkeletonCrossSize = AnimatedMesh->getBoundingBox().getExtend().X / 20.0f;
	}
	else
		DebugSkeletonCrossSize = OriginalMesh.getBoundingBox().getExtend().X / 20.0f;
}
